...
class CRUDReview(CRUDBase[FavoriteModel, FavoriteCreate, FavoriteUpdate]):
    async def add_to_favorites(self, db: AsyncSession, user_id: int, book_id: int) -> FavoriteModel:
    ...
    async def get_user_favorites(self, db: AsyncSession, user_id: int, skip: int, limit: int) -> list[FavoriteModel] | None:
    ...
    async def remove_from_favorites(self, db: AsyncSession, user_id: int, book_id: int) -> bool:
        result = await db.execute(
            delete(FavoriteModel)
            .where(FavoriteModel.user_id == user_id, FavoriteModel.book_id == book_id)
        )
        await db.commit()
        return result.rowcount > 0
    
    async def is_book_in_favorites(self, db: AsyncSession, user_id: int, book_id: int) -> bool:
        result = await db.execute(
            select(FavoriteModel)
            .where(FavoriteModel.user_id == user_id, FavoriteModel.book_id == book_id)
        )
