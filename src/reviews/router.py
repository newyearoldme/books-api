...
async def create_review(
    ...
    **Requirements**:
    ...
    if not book:
        raise NotFoundException(detail="Book not found", resource_type="book", resource_id=review_data.book_id)
...
async def read_review(review_id: int, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    **Path parameters:**
    ...
    if not review:
        raise NotFoundException(detail="Review not found", resource_type="review", resource_id=review_id)
    return review
...
async def delete_review(
    ...
    **Path parameters:**
    ...
    <u>Note: Users can only delete their own reviews. This action is permanent and cannot be undone.</u>
    """ 
    review = await review_crud.get(db, review_id)
    if not review:
        raise NotFoundException(detail="Review not found", resource_type="review", resource_id=review_id)
    ...
    if review.user_id != current_user.id:
        raise ForbiddenException(detail="Can only delete your own reviews")
