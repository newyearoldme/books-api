...
class CRUDBook(CRUDBase[BookModel, BookCreate, BookUpdate]):
    async def get_by_title(self, db: AsyncSession, title: str) -> BookModel:
        result = await db.execute(select(self.model).where(self.model.title == title))
        return result.scalar_one_or_none()
    
    async def get_by_author(self, db: AsyncSession, author: str) -> list[BookModel]:
        result = await db.execute(select(self.model).where(self.model.title == author))
        return result.scalars().all()
    
    async def get_top_rated(self, db: AsyncSession, limit: int = 10) -> list[BookModel]:
