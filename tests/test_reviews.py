...
@pytest.mark.asyncio
async def test_create_review(async_client, regular_user, regular_token, test_book):
    """Тест создания отзыва"""
    review_data = {"text": "Great book!", "rating": 5, "book_id": test_book["id"]}
...
@pytest.mark.asyncio
async def test_get_review_by_book(async_client, regular_user, regular_token, test_book):
    ...
    assert all(review["user_id"] == regular_user["id"] for review in response.json())


@pytest.mark.asyncio
async def test_get_reviews_by_user(
    async_client, regular_user, regular_token, test_book
):
    ...
    response = await async_client.get(f"/reviews/user/{regular_user['id']}")
...
@pytest.mark.asyncio
async def test_get_average_rating(async_client, regular_token, test_book):
    reviews_data = [
...
@pytest.mark.asyncio
async def test_create_review_nonexistent_book(async_client, regular_token):
...
@pytest.mark.asyncio
async def test_update_own_review(async_client, regular_token, test_book):
