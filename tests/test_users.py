...
@pytest.mark.asyncio
async def test_get_user_by_email(async_client, admin_token):
    ...
    response = await async_client.get(
        f"/users/by_email/{email}", headers={"Authorization": f"Bearer {admin_token}"}
    )
...
@pytest.mark.asyncio
async def test_get_users(async_client, admin_token):
...
@pytest.mark.asyncio
async def test_update_user_password(async_client, admin_token):
